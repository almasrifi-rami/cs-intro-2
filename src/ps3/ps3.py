...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # check if the room dimensions are valid
        if self.width > 0 and self.height > 0:
            # check if the dirt_amount is valid
            if self.dirt_amount >= 0:
                # dict to store tiles and their dirt amount
                self.tiles = {}
                # loop over the x and y to create each tile in the room
                for x in range(self.width):
                    for y in range(self.height):
                        # all tiles will have dirt_amount equal
                        self.tiles[(x, y)] = self.dirt_amount
            else:
                raise ValueError("Dirt amount cannot be negative!")
        else:
            raise ValueError("One or both of room dimensions is invalid!")
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        # get the tile x and y values from pos
        x, y = math.floor(pos.get_x()), int(pos.get_y())
        # update the tile dirt amount by the capacity
        try:
            self.tiles[(x, y)] = max(0, self.tiles[(x, y)] - capacity)
        except KeyError:
            print("Position is not inside the room!")
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        # initiate a variable to count cleaned tiles
        cleaned_tiles = 0
        # loop over all tiles keys and check whether they are cleaned
        for k in self.tiles.keys():
            m, n = k
            cleaned_tiles += self.is_tile_cleaned(m, n)
    ...
    def is_position_in_room(self, pos):
        ...
        y = pos.get_y()
        lower_limit = y >= 0 
        upper_limit = y < self.height
        left_limit = x >= 0
        right_limit = x < self.width
        return lower_limit and upper_limit and left_limit and right_limit
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        # use a random position in the room as the initial robot position 
        self.position = room.get_random_position()
        # get a random angle (direction) between 0 and 360
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        return len(list(self.tiles.keys()))
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return len(list(self.tiles.keys())) - len(self.furniture_tiles)
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        # list all unfurnished tiles
        unfurnished_tiles = [tile for tile in self.tiles.keys() \
                             if tile not in self.furniture_tiles]
        # select a random tile (x, y)
        x, y = random.choice(unfurnished_tiles)
        # then add a random float value (between 0 and 1)
        # to stay in the selected tile but add a random position inside
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_position = self.position.get_new_position(self.direction,
                                                      self.speed)
        # if the new position is valid, set it as the new robot position and
        # clean the room
        ...
        # if not, only change the robot direction
        else:
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # if the robot is faulty change directions
        if self.gets_faulty():
            self.set_robot_direction(random.uniform(0, 360))
        # if the robot is not faulty use the same method from StandardRobot
        else:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount,
                   min_coverage, num_trials, robot_type, animate=False,
                   room_type=EmptyRoom):
    """
    ...
    Returns:
        float: A tuple of mean number of time steps and 
    """
    # initiate a list to store the result from each trial
    trial_time_steps = []
    # create a loop with the number of trials specified
    for _ in range(num_trials):
        if animate:
            delay = 0.2
            if room_type == EmptyRoom:
                is_furnished = False
            elif room_type == FurnishedRoom:
                is_furnished = True
            anim = ps3_visualize.RobotVisualization(num_robots, width, height,
                                                    is_furnished, delay)
        # initiate a counter for the number of time steps at 0
        time_steps = 0
        # create an empty room with the given parameters
        room = room_type(width, height, dirt_amount)
        # create a list of robots with the type and given parameters
        robots = [robot_type(room, speed, capacity) for i in range(num_robots)]
        # create an infinite loop (because the condition is calculated later)
        while True:
            # at each step, calculate the ratio of cleaned tiles
            cleaned_tiles_ratio = room.get_num_cleaned_tiles() / \
                room.get_num_tiles()
            # compare to the minimum coverage and exit the loop if exceeded
            if cleaned_tiles_ratio >= min_coverage:
                break
            # otherwise loop over the robots and step clean
            else:
                for robot in robots:
                    robot.update_position_and_clean()
                if animate:
                    anim.update(room, robots)
                # increment the time steps by 1
                time_steps += 1
        # append the final number of time steps to the list
        trial_time_steps.append(time_steps)
        if animate:
            anim.done()
    # calculate and return the average time steps
    return sum(trial_time_steps) / num_trials
...
#       of a 20x20 room?
# The performance of 1 standard robot is higher than a faulty robot when cleaning
# 80% of the room by approx. 20%. However, the gap in performance become closer
# when comparing a larger number of robots of each type

# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
# Rooms that are square or close to square are easier to clean by both robots
# The bigger the aspect ratio is, the longer it takes to clean the room
# Standard robots are less affected by rectangular than Faulty robots
# due to the fact that there are many direction changes included
...
# run_simulation(2, 1.0, 1, 10, 10, 3, 0.8, 50, StandardRobot, animate=True)